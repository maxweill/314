...
int main(int argc, char *argv[])
{
	...
	//here we should be able to run the inner loop in parallel
	
	#pragma omp parallel for schedule(static,wl_size/4) private(j,hash) shared(wl_size,bv_size,wl,num_hf,bv)
	for (i = 0; i < wl_size; i++) {
		for (j = 0; j < num_hf; j++) {
